                      ...
                      {operation === 'update' && (formData.sn || formData.fnum) && (
                        <div className="bg-red-50 dark:bg-red-950/40 p-3 rounded-lg border border-red-200 dark:border-red-900 space-y-2 mb-4 shadow-sm">
                          ...
                          <div className="flex space-x-2">
                              <select value={archiveReason} onChange={(e) => setArchiveReason(e.target.value)} className="flex-1 text-xs border-red-300 dark:border-red-900 rounded shadow-sm border py-1.5 px-2 font-bold text-red-700 dark:text-red-400 outline-none focus:ring-1 focus:ring-red-400 bg-white dark:bg-slate-800">
  ...
  <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row justify-between items-center gap-3">
    ...
    {bulkSelectMode && selectedOfficers.length > 0 && (
        <div className="flex items-center space-x-2 animate-in fade-in slide-in-from-left-4 bg-white dark:bg-slate-900 p-1.5 rounded-lg shadow-sm border border-red-200 dark:border-red-900">
            {/* 🟢 Live Counter Badge */}
            <span className="text-xs font-black text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950 px-2.5 py-1 rounded">
              {selectedOfficers.length} {selectedOfficers.length === 1 ? 'Officer Selected' : 'Officers Selected'}
            </span>
            <select
                value={bulkArchiveReason}
                onChange={(e) => setBulkArchiveReason(e.target.value)}
                className="text-xs border border-slate-300 dark:border-slate-700 rounded p-1.5 outline-none font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800"
            >
