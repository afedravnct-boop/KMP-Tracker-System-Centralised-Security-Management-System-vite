...
import { 
  Sparkles, Send, Bot, User, Loader2, Minimize2, Maximize2, 
  ChevronDown, Database, FileText, Check, Copy, ShieldAlert 
} from 'lucide-react';
import { authFetch } from './api';

// Lightweight Markdown & Text Styler
...
const SystemAssistant = ({ currentUser, canViewGlobal }) => {
  ...
  const [messages, setMessages] = useState([
    { 
      sender: 'ai', 
      text: `Greetings ${currentUser?.rank || 'Officer'} ${currentUser?.name || ''}. I am your KMP CSDMS Tactical Intelligence Assistant. I have active clearance to query nominal rolls, crime registers, SITREPs, and lockup matrices across your jurisdiction.`,
      metadata: null 
    }
  ]);
  ...
  return (
    <div className="fixed bottom-3 left-16 z-[99990] flex flex-col items-start font-sans select-none">
      {/* Expanded AI Command Console Box */}
      {isOpen && (
        <div 
        ...
        >
          ...
          <div className="bg-slate-100/90 px-3 py-1.5 flex items-center gap-1.5 overflow-x-auto shrink-0 border-b border-slate-200">
            ...
            {quickPrompts.map((q, i) => (
              <button
                ...
                disabled={loading}
                className="text-[10px] font-medium bg-white hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded-full border border-slate-300 transition shrink-0 cursor-pointer disabled:opacity-50"
              >
                {q}
              </button>
            ))}
          ...
          <div 
            ref={chatScrollRef}
            className="flex-1 overflow-y-auto p-3.5 space-y-3 bg-slate-50 text-[11px]"
          >
            {messages.map((m, idx) => (
              <div key={idx} className={`flex flex-col ${m.sender === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`flex items-start space-x-2 max-w-[90%] ${m.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  ...
                  <div className={`p-3 rounded-2xl relative group ${
                    m.sender === 'user' 
                      ? 'bg-[#596E47] text-white rounded-tr-none shadow-xs' 
                    ...
                    {m.sender === 'ai' && (
                      <button
                        ...
                        className="absolute top-2 right-2 text-slate-400 hover:text-slate-700 opacity-0 group-hover:opacity-100 transition cursor-pointer p-0.5"
                        title="Copy intelligence brief"
                      >
                ...
                {m.metadata && (
                  <div className="flex flex-wrap items-center gap-1.5 mt-1 ml-7 text-[9px] text-slate-500 font-mono">
                    {m.metadata.sql_executed && (
                      <span className="flex items-center gap-1 bg-slate-200/80 px-1.5 py-0.5 rounded border border-slate-300">
                        <Database size={9} className="text-sky-600" /> SQL Scanned ({m.metadata.structured_records_count || 0} rows)
                      </span>
                    )}
                    {m.metadata.semantic_chunks_retrieved > 0 && (
                      <span className="flex items-center gap-1 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200 text-amber-800">
                        <FileText size={9} className="text-amber-600" /> {m.metadata.semantic_chunks_retrieved} SITREP Chunks
                      </span>
                    )}
                  </div>
                )}
            ...
            {loading && (
              <div className="flex items-center space-x-2 text-slate-600 italic text-[11px] bg-slate-100/90 p-2.5 rounded-xl border border-slate-200 w-fit">
                ...
                <span>Running dual-path SQL & SITREP vector analysis...</span>
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSendMessage} className="p-2.5 bg-white border-t border-slate-200 flex items-center space-x-2 shrink-0">
            <input 
              ...
              placeholder="Ask for officer deployments, crime stats, or SITREPs..." 
            ...
            <button 
              ...
              className="bg-[#3a3225] hover:bg-black text-white p-2.5 rounded-xl transition cursor-pointer disabled:opacity-40 shadow-xs"
      ...
      <button 
        ...
        className={`flex items-center justify-center transition-all duration-300 ease-in-out cursor-pointer shadow-xl rounded-full border ${
          isOpen
            ? 'h-8 px-3 bg-amber-500 text-slate-950 border-amber-300 shadow-[0_0_12px_rgba(245,158,11,0.5)]'
            : isHovered
            ? 'h-8 px-3 bg-slate-900 text-white border-slate-700'
